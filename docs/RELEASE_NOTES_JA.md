...
#### 📖 詳細ヘルプ機能の強化 (@Option 対応)

Gradle 標準のヘルプタスク（`help --task`）に統合され、個々のオプション（プロパティ）の詳細な説明をコマンドラインから確認できるようになりました。

```sh
./gradlew help --task generateDiagrams
```

#### 🤖 AI エージェント（Cursor, Windsurf, Cline 等）との連携強化

AI エージェントがプロジェクトの構造やこのプラグインの使い方をより正確に理解し、自動的に図を生成しやすくするための **Agent Skills** 定義を追加しました。

- スキル定義: `agents/skills/code-atlas/SKILL.md`

### 🔧 変更内容

- `generateDiagrams` タスクの説明文を更新し、CLI オプション（`--`）とプロジェクトプロパティ（`-P`）の両方がサポートされていることを明記しました。
